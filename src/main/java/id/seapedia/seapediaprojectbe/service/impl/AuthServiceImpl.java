...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        try {
            ...
            if (userRepository.existsByUsername(request.getUsername())) {
                log.warn("[register]  username already taken: username={}", request.getUsername());
                throw new BadRequestException("Username already taken");
            }

            log.debug("[register]  checking email uniqueness: username={}", request.getUsername());
            if (userRepository.existsByEmail(request.getEmail())) {
                log.warn("[register]  email already registered: username={}", request.getUsername());
    ...
    public AuthResponse login(LoginRequest request) {
        try {
            ...
            try {
                ...
                log.warn("[login]  invalid credentials: username={}", request.getUsername());
            ...
            User user = userRepository.findByUsername(request.getUsername())
                    .orElseThrow(() -> {
                        log.warn("[login]  user not found: username={}", request.getUsername());
    ...
    public AuthResponse selectRole(SelectRoleRequest request, UUID userId) {
        try {
            ...
            User user = userRepository.findById(userId)
                    .orElseThrow(() -> {
                        log.warn("[selectRole]  user not found: userId={}", userId);
                        return new ResourceNotFoundException("User not found");
                    });
            ...
            if (!hasRole) {
                log.warn("[selectRole]  role not assigned to user: userId={} role={}", userId, request.getRole());
    ...
    public FinancialSummaryResponse getFinancialSummary(UUID userId) {
        ...
        try {
            ...
            log.warn("[getFinancialSummary]  error: {}", e.getMessage());
        }
